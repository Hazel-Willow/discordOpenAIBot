...
import logging
from typing import Any, Literal, Optional, Dict, Tuple  # NEW
...
# ----------------------------- NEW: session anchor map + helper -----------------------------
last_bot_msg_by_channel: Dict[int, Tuple[int, float]] = {}  # channel_id -> (msg_id, ts)

def _update_bot_anchor(msg: discord.Message) -> None:
    """Record this bot message as the latest session anchor for its channel."""
    try:
        last_bot_msg_by_channel[msg.channel.id] = (msg.id, discord.utils.utcnow().timestamp())
    except Exception:
        logging.exception("Failed to update bot anchor")
# -------------------------------------------------------------------------------------------
    ...
    while curr_msg != None and len(messages) < max_messages:
        ...
        async with curr_node.lock:
            if curr_node.text == None:
                ...
                try:
                    # ----------------------------- NEW parent resolution block -----------------------------
                    # 0) Always prefer explicit reply target if present
                    ...
                    if ref and getattr(ref, "message_id", None):
                        ...
                        # Helper: simple hotness detector (short window + #authors)
                        HOT_WINDOW_SECONDS = 120
                        HOT_AUTHOR_THRESHOLD = 3  # distinct authors (including bot)
                        ...
                        # Helper: session anchor to bot's last message (optional heuristic)
                        session_parent = None
                        session_info = last_bot_msg_by_channel.get(ch.id)
                        if session_info:
                            session_parent_id, session_parent_ts = session_info
                            if (discord.utils.utcnow().timestamp() - session_parent_ts) <= 300:  # 5 min window
                                try:
                        ...
                        if is_dm_like:
                            ...
                            # Regular guild text channels: treat like public threads
                            if bot_mentioned:
                                curr_node.parent_msg = session_parent
                            else:
                                curr_node.parent_msg = session_parent if (session_parent and not is_hot) else None
                    # --------------------------- end NEW parent resolution block ---------------------------
                except (discord.NotFound, discord.HTTPException):
